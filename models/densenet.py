...
class _DenseLayer(nn.Module):
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate):
        ...
        :param bn_size:             让1x1卷积产生4k个特征图，达到降维的作用, 中间层growth rate 的拓展倍数
        :param drop_rate:           DropOut层的丢弃概率
        """
        super(_DenseLayer, self).__init__()

        # 处理顺序：BN-ReLU-Conv(1x1)-BN-ReLU-Conv(3x3)，称为DenseNet-B
...
class _Transition(nn.Sequential):    #通过1*1的卷积层减少通道数，并使用步幅为2的平均池化层减半高和宽，降低模型的复杂度
...
if __name__ == '__main__':
    inputs = torch.randn(1, 3, 224, 224)  #返回由可变参数大小定义的张量，其中填充了标准正态分布的值，即均值为0而方差为1
